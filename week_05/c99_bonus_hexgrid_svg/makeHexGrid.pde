...
ArrayList<HexPoint> makeGrid(float radius) {
  ...
  int number_of_hexes = ceil(leftover_width/(radius*3)) - 1;
  float start_x_offset = -(number_of_hexes*(radius*3));
  
  float leftover_height = (height/2)-(hex_height/2);
  number_of_hexes = ceil(leftover_height/(hex_height)) - 1;
  float start_y_offset = -((number_of_hexes*(hex_height))+(hex_height/2));
  
  int counter = 0;
  for (float y = start_y_offset; y <= abs(start_y_offset)+(hex_height*0); y+=(hex_height/2)) {    
    boolean shift_hex = false;
    if (counter % 2 == 0) {
      shift_hex = true;
    }
    for (float x = start_x_offset; x <= abs(start_x_offset)-(radius*0.1); x+=(radius*3)) {
      float newX = x;
      if (shift_hex) {
        newX += radius*1.5;
      }
